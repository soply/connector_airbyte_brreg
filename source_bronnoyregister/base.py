...
class BRREGBatchStream(HttpStream, ABC):
    ...
    def parse_response(self, response: requests.Response, stream_state: Mapping[str, Any], 
        stream_slice: Mapping[str, Any] = None, next_page_token: Mapping[str, Any] = None,) -> Iterable[Mapping]:
        """
...
class BRREGUpdateStream(BRREGBatchStream, ABC):
    ...
    def request_kwargs(
        ...
        if len(stream_state.keys()) == 0:
            # This branch covers the initial fetch, where we use the batch stream approach
            return super().request_kwargs(stream_state, stream_slice, next_page_token)
        else:
            # In incremental / update mode we do not need to return additional kwargs
            return {}
    ...
    def request_params(
        ...
        if len(stream_state.keys()) == 0 or next_page_token is not None:
            ...
            # 1) stream_state is empty, which is the case for the initial run
